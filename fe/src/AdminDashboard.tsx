...
import { api } from './api';
import type { ContentWithUser, SocialAccount, Content } from './types';
...
export function AdminDashboard() {
  ...
  const loadCreatorData = async () => {
    try {
      ...
      setCreatorError(err instanceof Error ? err.message : 'Failed to load data');
    } finally {
  ...
  const handleAddAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCreatorError(err instanceof Error ? err.message : 'Failed to add account');
    }
  };
  ...
  const handleDeleteAccount = async (id: number) => {
    ...
    try {
      ...
      setCreatorError(err instanceof Error ? err.message : 'Failed to delete account');
    }
  };
  ...
  const handlePullContent = async (accountId: number) => {
    try {
      ...
      setCreatorError(err instanceof Error ? err.message : 'Failed to pull content');
    }
  };
  ...
  const handleAddContent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCreatorError(err instanceof Error ? err.message : 'Failed to add content');
    }
  };
  ...
  const handleDeleteContent = async (id: number) => {
    ...
    try {
      ...
      setCreatorError(err instanceof Error ? err.message : 'Failed to delete content');
    }
  };
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="mb-8 animate-fade-in">
        <div className="flex gap-2 p-1 bg-slate-800/50 backdrop-blur-sm rounded-2xl border border-slate-700/50 w-fit">
          <button
          ...
          <button
            ...
            className={`px-6 py-3 rounded-xl font-semibold transition-all flex items-center gap-2 ${
              activeTab === 'creator'
                ? 'bg-gradient-to-r from-cyan-500 to-cyan-700 text-white shadow-lg shadow-cyan-500/25'
                : 'text-slate-300 hover:text-white hover:bg-slate-700/50'
            }`}
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {activeTab === 'admin' ? (
        ...
        <CreatorContent
          ...
          handlePullContent={handlePullContent}
...
interface CreatorContentProps {
  ...
  setShowContentForm: (show: boolean) => void;
  accountForm: { platform: string; account_name: string };
  setAccountForm: (form: { platform: string; account_name: string }) => void;
  contentForm: { platform: string; link: string; original_text: string; description: string; tags: string };
  setContentForm: (form: { platform: string; link: string; original_text: string; description: string; tags: string }) => void;
  handleAddAccount: (e: React.FormEvent) => void;
  handleDeleteAccount: (id: number) => void;
  handlePullContent: (accountId: number) => void;
  handleAddContent: (e: React.FormEvent) => void;
...
function CreatorContent({
  ...
  return (
    <>
      {/* Header */}
      <div className="mb-8 animate-fade-in">
        <h1 className="text-3xl sm:text-4xl font-bold mb-2 bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent">
          My Content
        </h1>
      ...
      <section className="mb-10">
        ...
        {showAccountForm && (
          <form onSubmit={handleAddAccount} className="bg-slate-800/50 backdrop-blur-sm rounded-2xl p-6 mb-6 border border-slate-700/50">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-900/50 border border-slate-600 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all"
                >
                  <option value="twitter">Twitter/X</option>
                  <option value="facebook">Facebook</option>
                  <option value="instagram">Instagram</option>
              ...
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full bg-slate-900/50 border border-slate-600 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all"
                />
              </div>
            </div>
            <button
              type="submit"
              className="w-full sm:w-auto px-6 py-3 bg-gradient-to-r from-cyan-500 to-cyan-700 text-white rounded-xl font-semibold shadow-lg shadow-cyan-500/25 hover:shadow-cyan-500/40 transition-all"
            >
              Link Account
            </button>
          </form>
        ...
        {socialAccounts.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {socialAccounts.map((account) => (
              <div
                key={account.id}
                className="bg-slate-800/50 backdrop-blur-sm rounded-2xl p-5 border border-slate-700/50 hover:border-cyan-500/30 transition-all group"
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-xl ${platformStyles[account.platform] || 'bg-slate-600'} flex items-center justify-center text-white shadow-lg`}>
                      {platformIcons[account.platform] || account.platform.charAt(0).toUpperCase()}
                    </div>
                    <div>
                ...
                <button
                  ...
                  className="w-full px-4 py-2 bg-slate-700/50 hover:bg-slate-700 text-slate-300 hover:text-white rounded-xl text-sm font-medium transition-all flex items-center justify-center gap-2"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <section>
        ...
        {showContentForm && (
          <form onSubmit={handleAddContent} className="bg-slate-800/50 backdrop-blur-sm rounded-2xl p-6 mb-6 border border-slate-700/50">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              <div>
                <label className="block text-sm font-medium text-slate-100 mb-2">Platform</label>
                <select
              ...
              <div>
                <label className="block text-sm font-medium text-slate-100 mb-2">Link *</label>
                <input
                  type="url"
            ...
            <div className="mb-4">
              <label className="block text-sm font-medium text-slate-100 mb-2">Original Text</label>
              <textarea
                value={contentForm.original_text}
                onChange={(e) => setContentForm({ ...contentForm, original_text: e.target.value })}
                placeholder="The original post content..."
                rows={3}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-100 mb-2">Tags</label>
                <input
                  type="text"
            ...
            <button
              type="submit"
              className="w-full sm:w-auto px-6 py-3 bg-gradient-to-r from-purple-500 to-purple-700 text-white rounded-xl font-semibold shadow-lg shadow-purple-500/25 hover:shadow-purple-500/40 transition-all"
            >
              Add Content
            </button>
          </form>
        ...
        {myContent.length === 0 ? (
          ...
          <div className="space-y-4">
            {myContent.map((item) => (
              <div
                key={item.id}
                className="bg-slate-800/50 backdrop-blur-sm rounded-2xl p-5 border border-slate-700/50 hover:border-purple-500/30 transition-all group"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-4 flex-1 min-w-0">
                    <div className={`w-10 h-10 rounded-xl ${platformStyles[item.platform] || 'bg-slate-600'} flex items-center justify-center text-white shadow-lg shrink-0`}>
                      {platformIcons[item.platform] || item.platform.charAt(0).toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                      <a 
                        ...
                        target="_blank" 
                        rel="noopener noreferrer"
                        className="text-cyan-400 hover:text-cyan-300 hover:underline font-medium block truncate"
                      >
                        {item.link}
                      </a>
                      {item.description && (
